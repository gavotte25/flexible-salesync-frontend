  ...
  return (
    ...
    >
      <Link
        ...
        to={`/private/section/home`}
      >
        <img
          src={`${import.meta.env.VITE_STORAGE_SERVICE_HOST}/companies/${companyLoaded?.avatar_url === 'default' || !companyLoaded?.avatar_url ? 'default.svg' : company?.avatar_url}?lastmod=${Date.now()}`}
          className=''
          alt='header icon'
          onError={(e) => {
