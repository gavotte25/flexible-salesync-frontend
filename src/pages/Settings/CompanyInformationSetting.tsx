...
const CompanyInfomationSetting = () => {
  ...
  const { company, updateCompanyInfo, uploadCompanyAvatar } = useAuth();
  ...
  const avatarUrl = `${import.meta.env.VITE_STORAGE_SERVICE_HOST}/companies/${companyLoaded.avatar_url === 'default' || !companyLoaded.avatar_url ? 'default.svg' : companyLoaded.avatar_url}`;
  ...
  const handleFileUpload = async (file: File) => {
    if (companyLoaded.company_id != undefined) {
      ...
      uploadCompanyAvatar(companyLoaded.name, companyLoaded.company_id, file).then(async (res) => {
        if (res) {
          console.log('Company avatar updated:', res);
          const newCompany = { ...companyLoaded, avatar_url: res };
